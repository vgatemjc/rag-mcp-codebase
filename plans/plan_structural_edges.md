...
1. Edge Package Setup [done]
1.1 [done] Create `server/services/edges/` with `__init__.py`, `edge_types.py`, `builder.py`, `plugins.py`.
1.2 [done] Codify edge enums/constants (per `plans/android_edge_types.md`: `BINDS_LAYOUT`, `NAV_DESTINATION`, `NAV_ACTION`, `NAVIGATES_TO`, `USES_VIEWMODEL`, `CALLS_API`, etc.) and payload schema (type/target/meta).
1.3 [done] Implement helpers for target normalization (ids, repo-relative paths), deduping, and validation.
1.4 [done] Add lightweight docstrings/type hints; keep ASCII only.

2. Android Integration
2.1 [done] Refactor `server/services/android_plugins.py` to emit edges through the shared builder (navgraph edges first, then layout binding) using the payload shapes in `plans/android_edge_types.md`.
2.2 [todo] Add Compose/navigation heuristics for `BINDS_LAYOUT`/`NAVIGATES_TO` when navgraph absent; include `USES_VIEWMODEL` from data-binding/ViewModel detection.
2.3 [done] Normalize ids to lowercase, strip `@+id/`, and use `layout/<file>.xml` targets.
